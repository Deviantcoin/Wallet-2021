...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "dev.conf"));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 22618, 51477));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all DEV specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 22617, 12272));
...
/** Sanity checks
 *  Ensure that DEV is running in a usable environment with all
...
#endif
    // Make sure only a single DEV process is using the data directory.
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                //DEV specific: zerocoin and spork DB's
