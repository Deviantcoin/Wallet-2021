...
void PrivacyDialog::setBalance(const CAmount& balance, const CAmount& unconfirmedBalance, const CAmount& immatureBalance, const CAmount& zerocoinBalance, const CAmount& unconfirmedZerocoinBalance, const CAmount& immatureZerocoinBalance, const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    ui->labelzAvailableAmount_4->setText(QString::number(zerocoinBalance / COIN) + QString(" zDEV "));
    ...
    ui->labelZsupplyAmount_2->setText(QString::number(chainActive.Tip()->GetZerocoinSupply() / COIN) + QString(" <b>zDEV </b> "));
...
void PrivacyDialog::updateSPORK16Status()
{
    ...
    if (fMaintenanceMode && fButtonsEnabled) {
        ...
        ui->pushButtonMintzfls->setToolTip(tr("zDEV is currently disabled due to maintenance."));

        // Spend zfls
        ui->pushButtonSpendzfls->setEnabled(false);
        ui->pushButtonSpendzfls->setToolTip(tr("zDEV is currently disabled due to maintenance."));
    } else if (!fMaintenanceMode && !fButtonsEnabled) {
        // Mint zfls
        ui->pushButtonMintzfls->setEnabled(true);
        ui->pushButtonMintzfls->setToolTip(tr("PrivacyDialog", "Enter an amount of DEV to convert to zDEV", 0));
