...
std::string AddressToString(const CBitcoinAddress& Address)
{
    ...
    /*
    CScript AddreDEVcript;
    AddreDEVcript.SetDestination(Address.Get());
    ...
    else
    {
        ...
        paddressmap->GetTxs(Txs, AddreDEVcript.GetID());
        BOOST_FOREACH (const CDiskTxPos& pos, Txs)
        {
            ...
            TxContent += TxToRow(tx, AddreDEVcript, Prepend, &Sum);
        }
    }
    */
    TxContent += "</table>";
...
void BlockExplorer::showEvent(QShowEvent*)
{
    if (m_NeverShown) {
        ...
        if (!GetBoolArg("-txindex", true)) {
            QString Warning = tr("Not all transactions will be shown. To view all transactions you need to set txindex=1 in the configuration file (deviant.conf).");
