...
#include "qt/dev/privacywidget.h"
#include "coincontrol.h"
#include "coincontroldialog.h"
#include "guiutil.h"
#include "optionsmodel.h"
#include "qt/dev/forms/ui_privacywidget.h"
                                               ...
                                               ui(new Ui::PrivacyWidget)
{
    ...
    ui->labelValueDenom1->setText("0x1 = 0 zDEV");
    ...
    ui->labelValueDenom5->setText("0x5 = 0 zDEV");
    ...
    ui->labelValueDenom10->setText("0x10 = 0 zDEV");
    ...
    ui->labelValueDenom50->setText("0x50 = 0 zDEV");
    ...
    ui->labelValueDenom100->setText("0x100 = 0 zDEV");
    ...
    ui->labelValueDenom500->setText("0x500 = 0 zDEV");
    ...
    ui->labelValueDenom1000->setText("0x1000 = 0 zDEV");
    ...
    ui->labelValueDenom5000->setText("0x5000 = 0 zDEV");
    setCssProperty(ui->labelValueDenom5000, "text-body2");

    ui->layoutDenom->setVisible(false);

    // List
    ui->labelListHistory->setText(tr("Last zDEV Movements"));
...
void PrivacyWidget::onMintSelected(bool isMint)
{
    QString btnText;
    if (isMint) {
        btnText = tr("Mint zDEV");
        ui->btnCoinControl->setVisible(true);
        ui->labelSubtitleAmount->setText(tr("Enter amount of DEV to mint into zDEV"));
    } else {
        btnText = tr("Convert back to DEV");
        ui->btnCoinControl->setVisible(false);
        ui->labelSubtitleAmount->setText(tr("Enter amount of zDEV to convert back into DEV"));
    }
...
void PrivacyWidget::onSendClicked()
{
    ...
    if (sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        warn(tr("Zerocoin"), tr("zDEV is currently undergoing maintenance"));
        return;
    }
    ...
    if (!ctx.isValid()) {
        inform(tr("You need to unlock the wallet to be able to %1 zDEV").arg(isConvert ? tr("convert") : tr("mint")));
        return;
    }
...
void PrivacyWidget::mint(CAmount value)
{
    ...
    if (!walletModel->mintCoins(value, CoinControlDialog::coinControl, strError)) {
        ...
        // Mint succeed
        inform(tr("zDEV minted successfully"));
        // clear
...
void PrivacyWidget::spend(CAmount value)
{
    ...
    if (!walletModel->convertBackzDEV(
        ...
        // Spend succeed
        inform(tr("zDEV converted back to DEV"));
        // clear
        ui->lineEditAmount->clear();
    }
}
