...
Enabling major improvements over the keystore management, the DEV wallet doesn't require regular backups as before, keys are following a deterministic creation path that can be verified at any time (before HD Wallet, every keypair was randomly created and added to the keypool, forcing the user to backup the wallet every certain amount of time or could end up loosing coins forever if the latest `wallet.dat` was not being used).
...
Automatic zDEV backup has been disabled. Thus, the following configuration options have been removed  (either as entries in the DEV.conf file or as startup flags):
- `autozDEVbackup`
- `backupzDEV`
- `zDEVbackuppath`
...
The default value for the stake-split threshold has been lowered from 2000 DEV, down  to 500 DEV.
...
- "moneysupply" and "zDEVSupply" attributes in `getblock` output are removed.
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/Deviant-Core-translations/), the QA team during Testing and the Node hosts supporting our Testnet.
